...
对 `.java` 文件得到 `java.md`，对 `.py`、`.pyi` 或 `.ipynb` 文件得到 `python.md`，对未识别扩展名
...
通过全部六项检查的文件才发给 LLM，除非仅 diff 本身就超过 `max_tokens` 的 80%：
`selectFiles` 在各检查之后施加该上限，并把文件排除为 `too_large`。它同样把新路径
为 `/dev/null` 的文件标记为 `deleted`（`deleted` 不是检查项——它是单独计算的）；
没有新内容可评审。用 `ocr review
