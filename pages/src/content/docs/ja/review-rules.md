...
システム側は**ファイルごと**に、[ファイルごとのルール解決](#ファイルごとのルール解決)の埋め込みテーブルから解決されます。1 つの catch-all `**/*` エントリで、`.java` ファイルには `java.md`、`.py`、`.pyi`、または `.ipynb` ファイルには `python.md`、認識できない拡張子には `default.md` が得られます。`merge_system_rule` は 3 つのユーザー層すべて（`--rule`、`<repo>/.opencodereview/rule.json`、`~/.opencodereview/rule.json`）で機能します。
